...
<<twitter_adoption_country, echo=FALSE, cache=FALSE, message=FALSE, results='asis', fig.width=5, fig.height=5, fig.align='center'>>=
library(xtable)
library(sqldf)
...
df_ta_c <- sqldf("SELECT nationality,
                 on_twitter,
                 off_twitter,
                 ((on_twitter*1.0)/(on_twitter + off_twitter)) * 100 AS percent_on,
                 tweets
                 FROM df_ta_c_
                 GROUP BY 1,2,3,5")
print(xtable(df_ta_c[with(df_ta_c, order(-percent_on)),], digits=c(0,0,0,0,0,0)), include.rownames=FALSE)
@
...
<<twitter_adoption_party, echo=FALSE, cache=FALSE, message=FALSE, results='asis', fig.width=5, fig.height=5, fig.align='center'>>=
library(xtable)
library(sqldf)
...
df_ta_e <- sqldf("SELECT european_party,
                 ...
                 FROM df_ta_e_
                 GROUP BY 1,2,3,5")
print(xtable(df_ta_e[with(df_ta_e, order(-percent_on)),], digits=c(0,0,0,0,0,0)), include.rownames=FALSE)
@
These are the most prolific users during the time frame of our study. It's interesting because this doesn't perfectly line up with the overall most prolific users from what I've seen. I'm having an issue with the date format from age. I will add it to this table as soon as I resolve the issue.
...
<<twitter_adoption_user_plot, echo=FALSE, cache=FALSE, message=FALSE, fig.width=5, fig.height=5, fig.align='center'>>=
...
<<twitter_adoption_user_replies, echo=FALSE, cache=FALSE, message=FALSE, results='asis', strip.white=TRUE>>=
library(xtable)
library(sqldf)
df_ta_u2 <- sqldf("SELECT name
                 , nationality
                 , CAST(STRFTIME('%Y.%m%d', 'now') - STRFTIME('%Y.%m%d', born) AS int) AS age
                 , SUM(tweets) AS tweets
                 , SUM(retweets) AS retweets
               ...
               FROM df
               GROUP BY 1,2,3
               ORDER BY name_replies DESC
               LIMIT 10")
print(xtable(df_ta_u2[with(df_ta_u2, order(-name_replies)),]), include.rownames=FALSE, floating=FALSE)
@
...
<<twitter_cor_tweets_replies, echo=FALSE, cache=FALSE, message=FALSE, fig.width=5, fig.height=5, fig.align='center'>>=
plot(df_ta_tr$tweets, df_ta_tr$replies)
abline(lm(df_ta_tr$tweets~df_ta_tr$replies), col="red") #regression
lines(lowess(df_ta_tr$tweets, df_ta_tr$replies), col="blue") #lowess
@
\end{document}