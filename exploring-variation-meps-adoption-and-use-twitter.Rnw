...
\maketitle
\abstract{Web 2.0 technologies like Twitter can enable more frequent and more substantive communication between political representatives and citizens. These opportunities should be particularly attractive to members of the European Parliament (MEPs). After all, the EU in general (Meyer 1999) and the EP in particular (Anderson and McLeod 2004) have been accused of being plagued by “communication deficits.” Turnout for EP elections remains low (Hobolt 2014; Franklin and Hobolt 2011; Mattila 2003), EP elections are generally seen as “second-order” (Marsh 2008; Schmitt 2005; but see Stockemer 2012), and citizens frequently fail to follow and recall EP-related news (Eurobarometer 79.5, 2013).

Despite the latter facts, there is significant variation in the rates and modalities of Twitter use among MEPs. This paper explores that variation. We analyze an original database of Twitter adoption and use by all MEPs represented in the 2014-2019 Parliament. We address three research questions. First, what factors affect the timing of MEPs’ decisions to join Twitter? Second, why do some MEPs tweet more frequently than others? And third, what factors explain the different modalities of MEPs’ Twitter use? With regard to the third question, we are interested in why some MEPs engage conversationally with stakeholders (e.g., through @replies) while others primarily use the service as a channel of top-down communication.

Studies of Twitter use among national legislators (Ausserhoffer and Maireder 2013; Chi and Yang 2010; Glassman, Strauss, and Shogan 2009; Larsson and Moe 2013; Peterson 2012; Shogan 2010; Williams and Gulati 2010) are widespread, and a number of authors have examined the use of Twitter in national EP election campaigns (Vergeer, Hermans, and Sams 2011; Vesnic-Alujevic 2011). We test the hypotheses that emerge from these studies in the paper. Our study is distinctive, however, in terms of its scope and its focus—we cover all MEPs (and can test to see, for example, whether various nationally specific variables matter), and we focus on Twitter as a representational tool rather than a campaigning tool.\\
...
\section*{Twitter: An Attractive Representational Tool for MEPs?} 
...
\section*{Data and Methods}
    ...
    Our statistical models include four dependent variables, which gauge the extent and nature of MEP (non)-engagement with Twitter. First, we determine whether an MEP had a Twitter account as of September 1, 2014 (0=no, 1=yes). Second, we calculate the length of time, in days, that each MEP has had a Twitter account, again using September 1, 2014 as a cut-off. Third, we count the number of tweets published by each MEP over the 88-day observed period. Fourth, to gauge the extent to which MEPs use Twitter as a means of direct dialogical engagement, we perform a count of each MEP's @-tweets over the examination period. 
    
    The indicators for our constituency-based independent variables are defined at the member-state level.\footnote{At present, six countries (Belgium, France, Ireland, Italy, Poland, and the United Kingdom) divide their territory into multiple constituencies for EP elections. While there is some evidence that their decision to do so might affect the relationships between MEPs and constituents \cite{bowler.farrell.1993, hix.2004}, most of our indicators are available only at the country level. Because of this data limitation, for example, all Belgian MEPs in our dataset take the overall Belgian World Economic Forum Global Information Technology score for the Global IT variable.} For \emph{District technology}, we integrate the World Economic Forum's Global IT scores for 2014 \cite{bilbao-osorio.dutta.lanvin.2014}. For \emph{District wealth}, we use 2013 Eurostat data on GDP per capita in purchasing power standards \cite{eurostat.2014}. For our \emph{District age} variable, we use estimated median ages as reported in the CIA World Factbook \cite{cia.2014}, and for \emph{District median freedom} we use the Reporters Without Borders World Press Freedom Index 2014 \cite{rsf.2014}.
    
   and S\&D party groups as "left" (1) and all MEPs from the ECR, EPP, and EFDD groups as "right" (0).\footnote{We code ALDE as "left" since the party group has traditionally championed the social principles--transparency, openness, and the centrality of citizen participation--upon which the intuition that "the left will use Twitter more" relies.} In addition, we code all non-attached MEPs according to the left-right ideology of their respective national parties (only three of the 52 non-attached MEPs are coded "left"). For \emph{Minority MEP}, we code members from party groups that overwhelmingly supported the Juncker Commission (EPP, S\&D, ALDE) as "governing" (0) and (a) members from groups that overwhelmingly rejected the Juncker Commission and (b) non-attached members as "opposition" (1). To test the notion that MEPs from less cohesive party groups will engage more with Twitter, we construct \emph{Inchoate MEP}. This indicator is constructed by subtracting the group's 2009-2014 Votewatch cohesion score \cite{thillaye.2014} from 100, which assures that MEPs from more inchoate groups have higher values. We code non-attached members as "missing" for this measure.
...
\section*{A Snapshot of MEPs' Twitter Use}
...
\section*{Discussion and Conclusions}
	...
	MEPs operate within one of the world's most dynamic legislatures. They routinely shape laws and policies that directly affect hundreds of millions of lives and indirectly affect billions of others. While their institution has gained significant power over the last 35 years, though, most MEPs still struggle to register on mass radars and to wrest the spotlight from national legislators who, in many cases, lack similar levels of influence. Twitter is certainly not the only way that MEPs can fight against publicity, communications, and democratic deficits, but it is one way.  This paper has presented the first systematic analysis of MEPs' engagement with Twitter and attempted to throw light on the factors that affect how MEPs are and are not using it.
	 ...
	 This study's analysis of strategic factors has suggested that MEPs may take to Twitter in an effort to "right a sinking ship," and, by extension, that Twitter activity might drop off once an MEP's party recovers. There is ample room for further investigations of the timing and rhythms of MEPs Twitter activity. Does MEP tweet activity follow the EU's rhythms, whee (for example) tweets increase in tandem with plenary weeks or European Council meetings? Or does activity track national rhythms, where (for example) Danish MEPs tweet furiously in the run up to Danish elections? In a slightly different extension, there is the potential for much theoretical purchase in comparison of MEPs and members of national parliaments.  Are their frequencies and styles similar or different, and why?  Answers to these kinds of questions would provide strong insights into the nature of Europe's multi-level polity and the distinctive (or not) models of representation and communication that distinguish MEPs from more familiar national politicians.
	 
	 Beyond the many research avenues that would effectively treat Twitter use or tweets as dependent variables, there are many potentially fecund questions that would inquire into the possible effects of Twitter use.  Does David Coburn's Twitter hyperactivity increase voters' familiarity with him? Does it increase citizen knowledge of the EP and/or the EU? Does it trigger more active engagement with other MEPs? With other EU institutions, with national politics? These are but a few of the questions that are ripe for future analysis. 
