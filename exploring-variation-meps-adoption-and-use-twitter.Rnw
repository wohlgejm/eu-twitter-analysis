...
@
<<descriptives_table, cache=TRUE, results='asis', echo=FALSE>>=
library(xtable)
xtable(summary(subset(df, select=c("tweets", "status_replies", "name_replies"))))
party <- aggregate(list(Tweets=df$tweets), by=list("European Party"=df$european_party), FUN=sum)
xtable(party)
gender <- aggregate(list(Tweets=df$tweets, "Status Replies"=df$status_replies, "Name Replies"=df$name_replies), by=list("Gender"=df$gender), FUN=sum)
xtable(gender)
@
<<plots, echo=FALSE>>=
hist(df$tweets)
